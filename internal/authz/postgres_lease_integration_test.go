...
func TestPostgresLeaseStoreLifecycle(t *testing.T) {
	...
	renewed, err := manager.Renew(context.Background(), LeaseRenewalRequest{LeaseID: lease.ID, TenantID: tenantID, FencingToken: lease.FencingToken, PrincipalID: lease.PrincipalID, PrincipalType: lease.PrincipalType, Role: lease.Role, PolicyVersion: lease.PolicyVersion, Grant: grant})
	if err != nil {
		t.Fatal(err)
	}
