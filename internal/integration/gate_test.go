...
func TestAuthoritativeGateRejectsCallerSuppliedAuditFacts(t *testing.T) {
	...
	task := state.ModuleTask{TenantID: "tenant-1", ProjectID: "project-1", ID: "task-1", State: contracts.TaskPassed, Version: 4, ModuleSpecRef: ref, AttemptSeriesID: "series-1", Attempt: 2}
	manifest := contracts.SubmissionManifest{SubmissionVersion: 1, ProjectID: "project-1", ModuleTaskID: "task-1", AttemptSeriesID: "series-1", Attempt: 2, ModuleSpecRef: ref, BaseCommit: commit(2), HeadCommit: commit(3), AgentIdentity: contracts.AgentIdentity{AgentInstanceID: "agent-1", Role: "EXECUTOR", LeaseID: "lease-1"}, CreatedAt: time.Now().UTC().Format(time.RFC3339), SHA256: digest("manifest")}
	gate, err := NewAuthoritativeGate(gateProjectSource{facts: ProjectFacts{TenantID: "tenant-1", ProjectID: "project-1", IntegrationID: "integration-1", BaseCommit: commit(1), PolicyDigest: digest("policy"), StateVersion: 7}}, gateTaskSource{task: task}, gateSubmissionSource{submission: repository.Submission{Manifest: manifest}}, gateEvidenceSource{evidence: EvidenceRecord{SHA256: digest("evidence"), ProjectID: "project-1", TaskID: "task-1", AttemptSeriesID: "series-1", Attempt: 2, ModuleSpecRef: ref, BaseCommit: commit(2), SubmissionCommit: commit(3), PolicyDigest: digest("policy"), Passed: true}}, gateModuleSource{module: ModuleRecord{ModuleID: "module-1", Ref: ref, OwnedPaths: []string{"owned/..."}, PublicInterfaces: []string{"HTTP /v1"}}}, gateAuthorizer{})
	if err != nil {
		t.Fatal(err)
	}
