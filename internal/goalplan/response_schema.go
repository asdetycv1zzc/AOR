...
const goalDraftSchema = `{
  ...
  "$defs": {
    ...
    "tool": {
      ...
      "allOf": [
        {"if": {"properties": {"source": {"const": "INSTALLED"}}, "required": ["source"]}, "then": {"required": ["inventoryId"], "not": {"required": ["install"]}}},
        {"if": {"properties": {"source": {"const": "INSTALL_REQUIRED"}}, "required": ["source"]}, "then": {"required": ["install"], "not": {"required": ["inventoryId"]}}}
      ]
    },
    "install": {
