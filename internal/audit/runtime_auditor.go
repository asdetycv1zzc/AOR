...
func (auditor *runtimeAuditor) prepare(ctx context.Context, input BlindAuditInput, refs ModuleAuditReferences) (preparedModuleAudit, error) {
	...
	return preparedModuleAudit{declaration: declaration, lease: toAgentLease(lease), modelCall: agentruntime.ModelCall{RequestID: stableModuleAuditID("module-audit-request-", input.AuditRunID, parameterDigest), Provider: route.Provider, Model: route.Model, ReasoningEffort: route.ReasoningEffort, ContextWindowTokens: route.ContextWindowTokens, CompactionThresholdTokens: route.CompactionThresholdTokens, ReservationID: stableModuleAuditID("module-audit-reservation-", input.AuditRunID, parameterDigest), MaxOutputTokens: route.MaxOutputTokens, ThinkingBudget: route.ThinkingBudget, Temperature: route.Temperature, Seed: cloneSeed(route.Seed), ProviderPolicy: route.ProviderPolicy, CachePolicy: route.CachePolicy, WorstCaseCostMicros: route.WorstCaseCostMicros, MaxAttempts: route.MaxAttempts}}, nil
}
