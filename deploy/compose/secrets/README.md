...
Files in this directory are local-only Compose secrets. Never commit their values. Compose requires all nine files below before it can start AOR:
...
Generate the local PostgreSQL and MinIO values and write actual issued model-provider credentials with the commands in the parent `README.md`. Keep this directory mode `0700`. File-backed Compose secrets must use mode `0444` so each container's non-root UID can read its mounted file; the non-traversable directory protects the source files on the host.
