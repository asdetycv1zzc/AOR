...
AS $$
  WITH global_capacity AS (
    SELECT greatest(8 - count(*) FILTER (
      WHERE task.state = 'EXECUTING' AND project.state = 'EXECUTING'
    ...
    SELECT task.tenant_id,
           ...
           row_number() OVER (
             PARTITION BY task.tenant_id, task.project_id
             ORDER BY task.priority DESC, task.critical_path_score DESC, task.updated_at, task.id
           ) AS project_rank,
           task.priority,
    ...
    SELECT ready.*,
           row_number() OVER (
             ORDER BY ready.project_rank, ready.priority DESC, ready.critical_path_score DESC, ready.updated_at,
                      ready.tenant_id, ready.project_id, ready.task_id
           ) AS global_rank
